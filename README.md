...
4. Use ansible-builder to create an ansible execution environment image (使用 ansible-builder 建立 execution environment 鏡像)
   ...
   ```
   - Create postgresql_ee container image 建構 postgresql_ee 容器鏡像
   ``` 
...
# Enable or disable DNS configuration, name of NiC, DNS upstream server 配置的啟用與否、網卡名稱、DNS 上游伺服器位址
dns_configure: true
interface: ens33
...
5. Use ansible to run automated configuration scripts (使用 ansible 運行自動化設定配置腳本)
``` 
ansible-navigator run -eei ['ee image name'] -mstdout 
```
