...
echo -e "[$(date)] \e[32mINFO\e[0m：operators_install.conf 配置檔確認完畢，開始執行 operators_install.sh"
...
define_global_env(){
  ...
  echo -e "[$(date)] \e[32mINFO\e[0m：define_global_env 執行完成"
}

# 創建 gitops repo
create_git_repo(){
  echo -e "[$(date)] \e[32mINFO\e[0m：開始執行 create_git_repo..."

  RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" -k GET "https://${GITEA_URL}/api/v1/repos/${GITEA_ADMIN}/OpenShift-EaaS-Practice")

  if [ "${RESPONSE}" = "200" ]; then
    echo -e "[$(date)] \e[32mINFO\e[0m：程式庫已存在！"
  else
    echo -e "[$(date)] \e[32mINFO\e[0m：程式庫未找到。建立 OpenShift-EaaS-Practice 程式庫"
  ...
  echo -e "[$(date)] \e[32mINFO\e[0m：create_git_repo 執行完成"
}

# 更新 gitops repo 內的參數
update_gitops_content(){
  echo -e "[$(date)] \e[32mINFO\e[0m：開始執行 update_gitops_content..."
  ...
  if [[ ! -d "${OCP_EAAS_DIR}/.bootstrap" ]]; then
    echo -e "[$(date)] \e[31mERROR\e[0m：找不到 .bootstrap 目錄，請確認該資料夾存在！"
    exit 1
  fi

  # 建立備份（首次才備份）
  if [[ ! -d "$BACKUP_EAAS_DIR" ]]; then
    echo -e "[$(date)] \e[32mINFO\e[0m：初始化備份目錄: $BACKUP_EAAS_DIR"
  ...
  # 還原備份
  echo -e "[$(date)] \e[32mINFO\e[0m：還原備份文件..."
  rsync -a --exclude=".git" "$BACKUP_EAAS_DIR/" "$OCP_EAAS_DIR/"

  # 開始替換操作
  echo -e "[$(date)] \e[32mINFO\e[0m：開始替換操作..."
  ...
  echo -e "[$(date)] \e[32mINFO\e[0m：替換操作完成！備份文件保存在: $BACKUP_EAAS_DIR"
  echo -e "[$(date)] \e[32mINFO\e[0m：update_gitops_content 執行完成"
}

# 將本地 git 推送至 gitea
push_git(){
  echo -e "[$(date)] \e[32mINFO\e[0m：開始執行 push_git..."
  ...
  echo -e "[$(date)] \e[32mINFO\e[0m：push_git 執行完成"
}

# 執行 gitops 自動化腳本
execute_gitops(){
  echo -e "[$(date)] \e[32mINFO\e[0m：開始執行 execute_gitops..."
  ...
  echo -e "[$(date)] \e[32mINFO\e[0m：execute_gitops 執行完成"
  echo -e "[$(date)] \e[32mINFO\e[0m：operators 建置完畢，請於 ArgoCD UI 確認部署完成"    
}
